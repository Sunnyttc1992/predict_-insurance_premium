...
class FeatureEngineering(BaseEstimator,TransformerMixin):
    ...
    """
    sklearn -compatible transformer encapsulating engineer features.
    ...
    def create_domain_features(self,df:pd.DataFrame) -> pd.DataFrame:
     ...
     def calculate_health_status(row):
            """Calculate health score based only on input features."""
            score = 100
            ...
            # Smoking impact
            if row['smoker'] == 'yes':
                score -= 30  # tuned to avoid being too predictive
